...
function w(r) {
  ...
  for (; s < r.length; ) {
    for (; !a(s) && s < r.length; )
      s++;
    for (o.push(r.slice(e, s)), e = s; !i(r[s]) && s < r.length; )
      s++;
...
async function C(r, e, t) {
  ...
  const n = document.querySelector("body"), i = g(n, "dialog");
  i && (i.close(), i.remove());
  const a = Object.assign(v(r, e), {
    component: r,
    cssClass: r,
    componentProps: e
  });
  return n.appendChild(a), await a.showModal(), a.addEventListener("keydown", $), t ? new Promise((o) => {
...
function v(r, e) {
  ...
  return e !== void 0 && Object.keys(e).forEach((a) => {
    n += ` data-${a}="${e[a]}"`;
  }), h.instance.configs.components.find((a) => a.name === r).presenterClassName && (n += ` data-presenter="${r}"`), n === "" ? t.innerHTML = `<${r}/>` : t.innerHTML = `<${r}${n}/>`, t.classList.add("modal", `${r}-dialog`), t;
}
function D(r, e) {
  const t = g(r, "dialog");
  if (e !== void 0) {
...
class L {
  ...
  async loadStyleSheets(e, t) {
    const n = [];
    return n.push(...e.map((i) => this.loadStyleSheet({
      cssText: i,
      identifier: t
  ...
  async loadStyleSheet({ url: e = null, cssText: t = null, identifier: n = null }) {
    if (!e && !t)
      return;
    const i = n || e;
    let a = this.loadedStyleSheets.get(i) || 0;
    if (a === 0)
      return new Promise((o, s) => {
        try {
          const l = document.createElement("style");
          l.textContent = t, n && (l.className = n), document.head.appendChild(l), this.loadedStyleSheets.set(i, a + 1), o(l.outerHTML);
        } catch (l) {
          s(new Error(`Failed to inject the CSS text: ${l.message}`));
        }
      });
    this.loadedStyleSheets.set(i, a + 1);
  }
  ...
  async loadComponent(e) {
    if (this.components[e.name]) {
      ...
      try {
        let t, n;
        e.directory ? (t = `./${h.instance.configs.webComponentsRootDir}/${e.directory}/${e.type}/${e.name}/${e.name}.html`, n = `./${h.instance.configs.webComponentsRootDir}/${e.directory}/${e.type}/${e.name}/${e.name}.css`) : (t = `./${h.instance.configs.webComponentsRootDir}/${e.type}/${e.name}/${e.name}.html`, n = `./${h.instance.configs.webComponentsRootDir}/${e.type}/${e.name}/${e.name}.css`);
        const i = e.loadedTemplate || await (await fetch(t)).text();
        this.components[e.name].html = i;
        const a = e.loadedCSSs || [await (await fetch(n)).text()];
        if (this.components[e.name].css = a, await this.loadStyleSheets(a, e.name), e.presenterClassName)
          if (e.presenterModule)
            this.registerPresenter(e.name, e.presenterModule[e.presenterClassName]);
          else {
            let o;
            e.directory ? o = `../../${h.instance.configs.webComponentsRootDir}/${e.directory}/${e.type}/${e.name}/${e.name}.js` : o = `../../${h.instance.configs.webComponentsRootDir}/${e.type}/${e.name}/${e.name}.js`;
            const s = await import(o);
            this.registerPresenter(e.name, s[e.presenterClassName]);
          }
        return this.components[e.name].isPromiseFulfilled = !0, { html: i, css: a };
      } catch (t) {
        throw t;
      }
    })();
  ...
  initialisePresenter(e, t, n, i = {}) {
    let a;
    try {
      a = new this.components[t.componentName].presenter(t, n, i), t.isPresenterReady = !0, t.onPresenterReady();
    } catch (o) {
...
class h {
  constructor() {
    this._appContent = {}, this.appServices = {}, this._documentElement = document, this.actionRegistry = {}, this.registerListeners(), this.ResourceManager = new L(), this.defaultLoader = document.createElement("dialog"), this.loaderCount = 0, this.defaultLoader.classList.add("spinner"), this.defaultLoader.classList.add("spinner-default-style"), window.showApplicationError = async (e, t, n) => await C("show-error-modal", {
      title: e,
      message: t,
      technical: n
  ...
  async loadConfigs(e) {
    try {
      ...
      for (const i of n.services) {
        const a = await import(i.path);
        this.initialiseService(a[i.name]);
      }
      for (const i of n.components)
        await this.defineComponent(i);
    } catch (t) {
  ...
  initialiseService(e) {
    let t = new e();
    Object.getOwnPropertyNames(e.prototype).filter((i) => i !== "constructor").forEach((i) => {
      this.appServices[i] = t[i].bind(t);
    });
  }
  showLoading() {
  ...
  async changeToDynamicPage(e, t, n, i) {
    try {
  ...
  validateTagName(e) {
    ...
    if (!this.configs.components.find((i) => i.name === e))
      throw new Error(`Element not found in configs: ${e}`);
  }
  async changeToStaticPage(e, t) {
    const n = this.showLoading();
    try {
      const i = await this.fetchTextResult(e, t);
      this.updateAppContent(i);
    } catch (i) {
      console.log("Failed to change page", i);
    } finally {
      this.hideLoading(n);
    }
  }
  ...
  preventExternalResources(e) {
    ...
    if (n)
      for (let i of n) {
        let a = i.split('"')[1], o = new URL(a).host;
        if (window.location.host !== o)
          throw new Error(`External resource detected: ${a}`);
      }
  }
  registerListeners() {
      ...
      for (; t && t !== this._documentElement && !n; ) {
        if (t.hasAttribute("data-local-action")) {
          ...
          for (; a === !1; ) {
            let d = !1, c;
            for (; d === !1; ) {
              if (i.webSkelPresenter) {
                d = !0, c = i.webSkelPresenter;
                break;
              }
              if (i = i.parentElement, i === document) {
            ...
            if (c[s] !== void 0)
              ...
              d = !1, i = i.parentElement;
          }
        } else if (t.hasAttribute("data-action")) {
          e.preventDefault(), e.stopPropagation(), n = !0;
          const i = t.getAttribute("data-action"), [a, ...o] = i.split(" ");
          a ? this.callAction(a, t, ...o) : console.error(`${t} : data action attribute value should not be empty!`);
          break;
        }
        t = t.parentElement;
      }
  ...
  callAction(e, ...t) {
    ...
    let i = t && t[0] instanceof HTMLElement ? t[0] : null;
    n.call(i, ...t);
  }
  async fetchTextResult(e, t) {
    ...
    if (!t) {
      const o = n + "#" + e;
      window.history.pushState({ relativeUrlPath: e, relativeUrlContent: a }, o.toString(), o);
    }
    return a;
  }
  /**
   ...
   */
  createElement(e, t = null, n = {}, i = {}, a = !1) {
    const o = document.createElement(e), { proxy: s, revoke: l } = this.createReactiveProxy(n, a, o);
    ...
    const d = {
      ...
      getOwnPropertyDescriptor(f, u) {
        return u === "element" ? {
          ...
          enumerable: !0,
          configurable: !1
        } : u in s ? Reflect.getOwnPropertyDescriptor(s, u) : u in o ? Reflect.getOwnPropertyDescriptor(o, u) : Reflect.getOwnPropertyDescriptor(f, u);
      }
    }, c = new Proxy({}, d);
    ...
    customElements.get(e.name) || customElements.define(
      ...
      class extends HTMLElement {
        ...
        async connectedCallback() {
          this._webSkelProps && (this.props = this._webSkelProps.proxy), this.resources = await h.instance.ResourceManager.loadComponent(e), y(this.resources.html).forEach((a) => {
            a = a.slice(2), this.variables[a] = "";
          }), this.templateArray = w(this.resources.html);
          let n = this, i = null;
          for (const a of n.attributes)
            n.variables[a.nodeName] = b(a.nodeValue), a.name === "data-presenter" && (i = a.nodeValue);
          if (i) {
            const a = async (s) => {
              const l = (c) => {
...
`)[1], console.error(c), h.instance.hideLoading();
            ...
            n.invalidateFn = o, n.webSkelPresenter = h.instance.ResourceManager.initialisePresenter(i, n, o, this.props);
          } else
            n.refresh();
        }
